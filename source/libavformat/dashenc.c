...
static int add_adaptation_set(AVFormatContext *s, AdaptationSet **as, enum FFAVMediaType type)
{
    DASHContext *c = s->priv_data;
    void *mem;
    ...
    while (*p) {
        if (*p == ' ') {
            ...
            if (as->media_type == AVMEDIA_TYPE_UNKNOWN && (idx_str[0] == 'v' || idx_str[0] == 'a')) {
                enum FFAVMediaType type = (idx_str[0] == 'v') ? AVMEDIA_TYPE_VIDEO : AVMEDIA_TYPE_AUDIO;
