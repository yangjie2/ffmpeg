...
static int init_input_filter(FilterGraph *fg, InputFilter *ifilter)
{
    ...
    enum FFAVMediaType type = ifp->type;
    int i, ret;
    ...
    if (ifp->linklabel) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            enum FFAVMediaType stream_type = s->streams[i]->codecpar->codec_type;
            if (stream_type != type &&
                       ...
                       const char *filter_name)
{
    ...
    AVFilterContext *ctx;
    const AVFilter *trim;
    enum FFAVMediaType type = avfilter_pad_get_type((*last_filter)->output_pads, *pad_idx);
...
static int fg_output_frame(OutputFilterPriv *ofp, AVFrame *frame)
{
    ...
    enum FFAVMediaType type = ofp->ofilter.type;
